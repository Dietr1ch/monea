...
Core tests take longer than any other Monea tests, due to the high amount of computational work involved in validating core components.
...
To run only Monea's core tests (after building):
...
To run only Monea's crypto tests (after building):
...
To run only Monea's hash tests (after building):
...
To run only Monea's performance tests (after building):
...
To run only Monea's unit tests (after building):
